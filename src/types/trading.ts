...
export interface Order {
  ...
  ///
  /// A cloid names exactly ONE order. The node refuses a repeat with
  /// `ORDER_DUPLICATE_CLOID`, and the rule applies per LEG: a `batchOrder`
  /// dedups every leg that carries one, and two legs of ONE action that share a
...
export type ApiErrorCode =
  ...
  | 'ACTION_UNSUPPORTED'
  | 'PRECONDITION_FAILED'
  /// The block builder dropped the action: this account already used the nonce,
  /// or the nonce sits more than 64 below the newest one it committed. Nothing
  /// committed. It arrives at HTTP 200, because it is a commit verdict and not
  /// an admission refusal — on an order action inside `statuses[0].error`, on
