...
///
/// An order your balance cannot fund at all is refused with `insufficient spot
/// balance`, and no order id is burned. The refusal is about money, not
/// liquidity: a funded order that finds no counterparty still answers `filled`
/// with `total_sz: "0"`. One exception is an accepted no-op: a market buy that
/// holds quote, when the pair carries no FOREIGN ask (an ask from another
/// account). A foreign ask your quote cannot buy one lot of is a refusal, not a
/// no-op.
///
