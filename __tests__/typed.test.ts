...
// something the chain will not verify.
// ============================================================================

const EXP_NONCE = 1_735_689_600_000n;
const EXP_SET = 1_735_693_200_000n;

describe.skipIf(!wasmBuilt)('typed action expiresAfter fold', () => {
  it('withdraw: expiry 0 is byte-identical to the plain digest (chain pin)', async () => {
    const { buildTyped, typedActionDigest } = await import('../src/native/typed.js');
    const payload = {
      asset: 0,
      amount: '100.5',
    ...
    const plain = await typedActionDigest(
      buildTyped('withdraw', payload, EXP_NONCE, CHAIN_ID),
    );
    const zero = await typedActionDigest(
      buildTyped('withdraw', payload, EXP_NONCE, CHAIN_ID, undefined, 0n),
    );
    // E2-withdraw, expiry 0 == chain plain digest.
  ...
  it('withdraw: non-zero expiry matches the chain-folded digest (chain pin)', async () => {
    const { buildTyped, typedActionDigest } = await import('../src/native/typed.js');
    const built = buildTyped(
      'withdraw',
      { asset: 0, amount: '100.5', destination_chain_id: 8453, use_cctp: false },
      EXP_NONCE,
      CHAIN_ID,
      undefined,
      EXP_SET,
    );
    // E2-withdraw, expiry set.
    expect(toHex(await typedActionDigest(built))).toBe(
      '9ad23a96bb83b8bdd427fe9023b4855e8689be66da73da745f9af0acb59f5833',
    );
  });

  it('folded encodeType splices ,uint64 expiresAfter before the closing paren', async () => {
    const { encodeType, foldExpiryTypeString } = await import('../src/native/typed.js');
    const base = encodeType('withdraw');
  ...
  it('every typed action: digest(expiresAfter=0n) === digest() byte-for-byte', async () => {
    const { buildTyped, typedActionDigest } = await import('../src/native/typed.js');
    for (const v of VECTORS) {
      const plain = await typedActionDigest(
        buildTyped(v.actionType, v.payload, v.nonce, CHAIN_ID),
      );
      const zero = await typedActionDigest(
        buildTyped(v.actionType, v.payload, v.nonce, CHAIN_ID, undefined, 0n),
      );
      expect(toHex(zero), `${v.actionType} expiry-0 must match plain`).toBe(
        toHex(plain),
      );
    }
  });

  it('typedRequestBody omits expires_after at 0, carries it when non-zero', async () => {
    const { signTypedAction, typedRequestBody } = await import('../src/native/typed.js');
    const privKey = new Uint8Array(32).fill(0x42);
    const payload = { asset: 0, amount: '100.5', destination_chain_id: 8453, use_cctp: false };
    const zero = await signTypedAction(privKey, 'withdraw', payload, EXP_NONCE, CHAIN_ID);
    expect(typedRequestBody(zero).includes('expires_after')).toBe(false);
    const set = await signTypedAction(
      privKey,
      'withdraw',
      payload,
      EXP_NONCE,
    ...
    const body = typedRequestBody(set);
    expect(body.includes(`"expires_after":${EXP_SET}`)).toBe(true);
    const parsed = JSON.parse(body) as { expires_after?: number };
    expect(parsed.expires_after).toBe(Number(EXP_SET));
  });

  it('typedDataV4 renders expiresAfter as the trailing uint64 only when set', async () => {
    const { buildTyped, typedDataV4 } = await import('../src/native/typed.js');
    const payload = { asset: 0, amount: '100.5', destination_chain_id: 8453, use_cctp: false };
    const zero = typedDataV4(buildTyped('withdraw', payload, EXP_NONCE, CHAIN_ID, undefined, 0n));
    const zeroFields = zero.types['MetaFluxTransaction:Withdraw'];
    expect(zeroFields?.some((f) => f.name === 'expiresAfter')).toBe(false);
    expect(zero.message.expiresAfter).toBeUndefined();
    const set = typedDataV4(
      buildTyped('withdraw', payload, EXP_NONCE, CHAIN_ID, undefined, EXP_SET),
    );
    const setFields = set.types['MetaFluxTransaction:Withdraw'];
    expect(setFields?.at(-1)).toEqual({ name: 'expiresAfter', type: 'uint64' });
    expect(set.message.expiresAfter).toBe(EXP_SET.toString());
  });

  it('sign → recover round-trips with a non-zero expiry', async () => {
    const { signTypedAction, recoverTypedSigner } = await import('../src/native/typed.js');
    const { deriveAddressFromPubkey, recoverPubkey } = await import('../src/wallet/wasm.js');
    ...
    expect(signed.expiresAfter).toBe(EXP_SET);
    const recovered = await recoverTypedSigner(signed, 'withdraw', payload, CHAIN_ID);
    // Recovers cleanly (same expiry threaded via signed.expiresAfter).
